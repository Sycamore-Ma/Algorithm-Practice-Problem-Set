...
for _ in range(t):
    ...
    total_time = [[INF] * len(graph[_]) for _ in range(n)];   total_time[0][0] = 0
    wait_time = [[INF] * len(graph[_]) for _ in range(n)];    wait_time[0][0] = 0
    ...
    while priority_queue:
        ...
        if total_u > total_time[u][current_index] or total_u == total_time[u][current_index] and wait_u > wait_time[u][current_index]:
            continue
        # if total_u != total_time[u][current_index] or wait_u != wait_time[u][current_index]:
        #     continue
